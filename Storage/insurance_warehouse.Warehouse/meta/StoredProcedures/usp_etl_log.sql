...
BEGIN
    ...
    -- Đồng bộ sử dụng hệ thời gian chuẩn xác của Fabric Warehouse
    ...
    BEGIN
        ...
        -- Cập nhật tiến trình watermark cho các bảng chạy tăng trưởng (Incremental)
    ...
    BEGIN
        -- Khắc phục lỗi UPDATE CROSS JOIN bằng cách tính toán tổng số dòng vào các biến cục bộ trước
        ...
        -- Thực hiện câu lệnh UPDATE đơn giản, chuẩn hóa hiệu năng cho Warehouse phân tán
        UPDATE meta.etl_execution_log
        SET status        = 'Success',
            end_time      = @now,
            rows_read     = @agg_read,
    ...
    -- ------------------------------------------------------------------------------------------
    -- TRẢ KẾT QUẢ ĐẦU RA (OUTPUT) DẠNG RECORDSET CHO FABRIC PIPELINE CÓ THỂ ĐỌC ĐƯỢC 100%
    -- ------------------------------------------------------------------------------------------
