...
# BK: cancellation_id
# Responsibility: cleanse, validate → silver.policy_cancellation

bronze_schema = "bronze"
silver_schema = "silver"
SOURCE_TABLE  = f"{bronze_schema}.policy_cancellation"
TARGET_SILVER = f"{silver_schema}.policy_cancellation"
BK_COL        = "cancellation_id"
...
df_cancel_src = spark.read.table(SOURCE_TABLE)

if _wc and _lw:
    df_cancel_src = df_cancel_src.filter(
...
df_cancel_silver = (
    df_cancel_src
    .select(
        F.upper(F.trim(F.col("cancellation_id"))).alias("cancellation_id"),
        F.upper(F.trim(F.col("policy_id"))).alias("policy_id"),
        F.to_timestamp(
            F.regexp_replace(F.col("cancellation_date"), "T", " "),
            "yyyy-MM-dd HH:mm:ss.SSS"
        ).alias("cancellation_date"),
        F.trim(F.col("cancellation_reason")).alias("cancellation_reason"),
    ...
    .filter(F.col("refund_amount") >= 0)          # per mapping doc: >= 0, khác payment > 0
    .filter(F.col("cancellation_id").isNotNull() & (F.trim(F.col("cancellation_id")) != ""))
)
...
df_cancel_silver.cache()
rows_inserted = df_cancel_silver.count()
