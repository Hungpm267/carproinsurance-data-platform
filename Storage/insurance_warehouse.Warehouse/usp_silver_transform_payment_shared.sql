CREATE OR ALTER PROCEDURE silver.usp_load_policy_payment
...
BEGIN
    ...
    ;WITH src AS (
        SELECT *
        FROM insurance_lakehouse.bronze.policy_payment
        WHERE @incremental = 0 OR last_updated > @last_watermark_dt
    ),
    deduped AS (
        SELECT *,
            ROW_NUMBER() OVER (PARTITION BY payment_id ORDER BY last_updated DESC) AS _rn
        FROM src
    ),
    cleansed AS (
        SELECT
            UPPER(TRIM(CAST(payment_id            AS VARCHAR(100))))  AS payment_id,
            UPPER(TRIM(CAST(policy_id             AS VARCHAR(100))))  AS policy_id,
            TRY_CAST(
                REPLACE(CAST(payment_date AS VARCHAR(50)), 'T', ' ')
            AS DATETIME2(6))                                          AS payment_date,
            TRIM(CAST(payment_method              AS VARCHAR(100)))   AS payment_method,
            UPPER(TRIM(CAST(payment_status        AS VARCHAR(100))))  AS payment_status,
            TRY_CAST(payment_amount               AS DECIMAL(18,2))   AS payment_amount,
            TRIM(CAST(transaction_reference       AS VARCHAR(4000)))  AS transaction_reference,
            TRY_CAST(
        ...
        WHERE _rn = 1
          ...
          AND TRIM(CAST(payment_id AS VARCHAR(100))) <> ''
    ...
    SET @rows_read = (
        SELECT COUNT(*) FROM insurance_lakehouse.bronze.policy_payment
        WHERE @incremental = 0 OR last_updated > @last_watermark_dt
    );
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            WHERE s.name = 'silver' AND t.name = 'policy_payment'
        )
        BEGIN
            SELECT * INTO silver.policy_payment FROM #cleansed;
        END
        ELSE
        BEGIN
            MERGE INTO silver.policy_payment AS tgt
            ...
            WHEN MATCHED THEN UPDATE SET
                ...
                tgt.transaction_reference = src.transaction_reference,
            ...
            WHEN NOT MATCHED THEN INSERT (
                payment_id, policy_id, payment_date, payment_method, payment_status,
                payment_amount, transaction_reference, last_updated, operation_type,
                batch_date, source_system
            )
            VALUES (
                src.payment_id, src.policy_id, src.payment_date, src.payment_method, src.payment_status,
                src.payment_amount, src.transaction_reference, src.last_updated, src.operation_type,
                src.batch_date, src.source_system
            );
        END
