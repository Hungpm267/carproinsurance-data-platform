...
df_customer = spark.table(f"{silver_schema}.crm_customer")

rows_read = df_customer.count()
...
silver_schema = "silver"
gold_schema   = "gold"
SOURCE_SILVER = f"{silver_schema}.crm_customer"
TARGET        = f"{gold_schema}.dim_customer"
SK_COL        = "customer_key"
BK_COL        = "customer_id"

CUSTOMER_COLS = [
    "customer_key", "customer_id", "full_name", "gender", "date_of_birth",
    "age", "phone_number", "email", "city", "district",
    "customer_since_date", "row_hash", "effective_date", "expiry_date", "is_current",
]
...
src_cols = [
    ...
    "customer_since_date", "row_hash", "created_date", "updated_at",
]
...
df_insert = generate_sk(df_insert_raw, TARGET, SK_COL).select(*CUSTOMER_COLS)
...
#  MERGE SOURCE 
merge_src = df_expire_updates.unionByName(df_insert)
merge_src.createOrReplaceTempView("src_dim_customer")
...
# MAGIC %%sql
# MAGIC --  9. MERGE 
# MAGIC -- MATCHED     → UPDATE
# MAGIC -- NOT MATCHED → INSERT
# MAGIC MERGE INTO gold.dim_customer AS tgt
# MAGIC USING src_dim_customer       AS src
# MAGIC ON tgt.customer_key = src.customer_key
# MAGIC 
...
# MAGIC     customer_key, customer_id, full_name, gender, date_of_birth, age,
# MAGIC     phone_number, email, city, district, customer_since_date,
# MAGIC     row_hash, effective_date, expiry_date, is_current
# MAGIC )
# MAGIC VALUES (
# MAGIC     src.customer_key, src.customer_id, src.full_name, src.gender,
# MAGIC     src.date_of_birth, src.age, src.phone_number, src.email,
# MAGIC     src.city, src.district, src.customer_since_date,
# MAGIC     src.row_hash, src.effective_date, src.expiry_date, src.is_current
# MAGIC );
