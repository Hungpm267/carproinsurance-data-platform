...
if USE_MOCK:
    df_policy_src = spark.createDataFrame([
        ("POL001", "Q001", "C001", "P001", "ACTIVE",    5_000_000.0),
        ("POL002", "Q002", "C002", "P002", "CANCELLED", 8_000_000.0),
    ], schema="policy_id STRING, quotation_id STRING, customer_id STRING, provider_code STRING, policy_status STRING, premium_amount DOUBLE")
else:
    df_policy_src = spark.read.table(f"{silver_schema}.policy_policy")
...
TARGET_TABLE = f"{gold_schema}.dim_policy_status"
SK_COL = "policy_status_key"
NAME_COL = "policy_status_name"
SOURCE_COL = "policy_status"
...
new_rows_sk.select(SK_COL, NAME_COL).createOrReplaceTempView("src_dim_policy_status")
...
# MAGIC %%sql
# MAGIC 
# MAGIC MERGE INTO gold.dim_policy_status AS tgt
# MAGIC USING src_dim_policy_status AS src
# MAGIC ON tgt.policy_status_name = src.policy_status_name
# MAGIC WHEN MATCHED THEN UPDATE SET
# MAGIC     tgt.policy_status_name = src.policy_status_name
# MAGIC WHEN NOT MATCHED THEN INSERT (policy_status_key, policy_status_name)
# MAGIC VALUES (src.policy_status_key, src.policy_status_name);
